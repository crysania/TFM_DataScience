...
```{r Bloque de prediccion}

logviajeros_ts <- log(viajeros_ts)

fit <- arima(logviajeros_ts, c(0, 1, 1), seasonal = list(order = c(0, 1, 1), period = 12))

pred <- predict(fit, n.ahead = 2 * 12) #predicción para los dos proximos año
...
#dibujamos el ACF y observamos que los datos no son estacionarios
acf(viajeros_ts, 50)
...
Se denomina tendencia de una serie temporal a su comportamiento o movimiento a largo plazo. Por ejemplo, la gráfica adjunta muestra la serie movimientos turísticos en España, desde Octubre del 2015 hasta los próximos dos años.

La línea recta podría representar la tendencia (creciente).
